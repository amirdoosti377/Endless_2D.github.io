...
export function drawAtmosphere(c,w,h,p,time,reduced,low=false){
  const t=reduced?0:time;
  for(let layer=0;layer<2;layer++)for(let i=0;i<(low?12:30);i++){
...
export function drawArenaEffects(c,world,time,reduced,alpha=1,visible=()=>true,low=false){
  const t=reduced?0:time;
  for(const h of world.hazards){
    if(!visible(h.x,h.y,h.radius+10))continue;
  ...
  if(!reduced)for(const r of world.rings){if(!visible(r.x,r.y,150))continue;c.globalAlpha=r.life/.45;c.strokeStyle=r.color;c.lineWidth=2;c.beginPath();c.arc(r.x,r.y,r.radius+(1-r.life/.45)*(r.pulse?100:44),0,Math.PI*2);c.stroke();}
  c.globalAlpha=1;
  if(!reduced&&!low)for(const s of world.trail){c.globalAlpha=s.life*.5;c.fillStyle='#5ef4d5';c.beginPath();c.arc(s.x,s.y,3+s.life*12,0,Math.PI*2);c.fill();}c.globalAlpha=1;
  // Each enemy has a locked, visible attack direction during its windup.
  for(const e of world.enemies){
    if(!visible(e.x,e.y,e.radius+40))continue;
  ...
  for(const b of world.hostile){const x=(b.px??b.x)+(b.x-(b.px??b.x))*alpha,y=(b.py??b.y)+(b.y-(b.py??b.y))*alpha;if(!visible(x,y,10))continue;c.strokeStyle='#ff84df';c.fillStyle='#ffe5f8';c.lineWidth=2;c.beginPath();c.arc(x,y,b.radius,0,Math.PI*2);c.fill();c.stroke();}
}
